...
my @ssuassem_results_sorted;	# Array to store sorted list of SSU sequences for reporting
...
sub bbmap_fast_filter_parse() {
    ...
    $SSU_total_pairs = $ssu_f_reads + $ssu_r_reads - $ssu_pairs - $ssu_bad_pairs;
    if ($SEmode == 0) {
...
sub emirge_run {
	...
	if ($ins_me < $min_ins) { 
	    msg("Warning: estimated insert size very small: $ins_me, using $min_ins instead");
	    $ins_used = $min_ins;
	} else {
	    $ins_used = $ins_me;
	}
...
sub write_report_html {			
    # Generate HTML-formatted report file -- lots of blocks of verbatim HTML in this section
...
foreach my $taxonshortstring ( @taxa_from_hitmaps_sorted ) {                                                            # For each of these higher taxa
  print {$fh} "  <tr>\n";
...
foreach (@ssuassem_results_sorted) {
	my @split_entry = @$_;
	my @get_genbank = split (/\./,$split_entry[2]);									# Parse the database entry number of the reference sequence to get Genbank accession
	print {$fh} "  <tr>\n";
